...
SDL_FRect world_to_screen(FRect *world_rect) {
  SDL_FRect screen_rect = {
      ...
      .x = (world_rect->x - render_context.camera.rect.x) * render_context.camera.zoom + render_context.window_data.dimensions.w / 2,
      .y = (world_rect->y - render_context.camera.rect.y) * render_context.camera.zoom + render_context.window_data.dimensions.h / 2,
  };
...
void draw_texture(int image_id, SDL_FRect *rendering_rect) {
  int copy_result = SDL_RenderCopyF(render_context.renderer, render_context.image_atlas.images[image_id].texture, NULL, rendering_rect);
  if (copy_result != 0) {
...
void log_entity_personalities(int entity_id) {
  for (int personality_i = 0; personality_i < Personality_Count; personality_i++) {
    if (Entity__has_personality(entity_id, personality_i)) {
      print(
          "Entity %s has personality %s with value %d", game_states.current_state.names[entity_id], Personality__Strings[personality_i],
...
void update(double delta_time) {
  ...
  entity_loop(entity_id) {
      game_states.current_state.rect[entity_id].x += game_states.current_state.direction[entity_id].x * (float)(delta_time * game_states.simulation_speed * game_states.current_state.speed[entity_id]);
      game_states.current_state.rect[entity_id].y += game_states.current_state.direction[entity_id].y * (float)(delta_time * game_states.simulation_speed * game_states.current_state.speed[entity_id]);
  }
}
...
void interpolate_states() {
  ...
  entity_loop(entity_id) {
      game_states.render_state.rect[entity_id].x = game_states.current_state.rect[entity_id].x * (float)game_states.alpha + game_states.previous_state.rect[entity_id].x * (float)(1.0 - game_states.alpha);
      game_states.render_state.rect[entity_id].y = game_states.current_state.rect[entity_id].y * (float)game_states.alpha + game_states.previous_state.rect[entity_id].y * (float)(1.0 - game_states.alpha);
  }
}
...
void render() {
  ...
  entity_loop(entity_id) {
    SDL_FRect rendering_rect = world_to_screen(&game_states.render_state.rect[entity_id]);
    draw_texture(game_states.render_state.image[entity_id], &rendering_rect);
    //printf("Entity Image ID: %d\n", game_states.current_state.image[entity_id]);
  }

  //u32 image_id = 0;
  //draw_texture(0, &(SDL_FRect){.x = 0.0f, .y = 0.0f, .w = (float)render_context.image_atlas.images[image_id].w, .h = (float)render_context.image_atlas.images[image_id].h});

  draw_debug_text(0, "Simulation: [Speed: %.2f]", game_states.simulation_speed);
...
void load_fonts() {
  ...
  //Until then, enjoy the magic numbers. 4 = paths count, 128 = max path length
  render_context.font_atlas.count = 1;
  char font_paths[1][128] = {
    "assets/OpenSans-Regular.ttf"
  };

  for(u32 i = 0; i < render_context.font_atlas.count; i++) {
...
void load_images() {
  ...
  //unfortunately platform dependent so it would also require some type of platform abstraction to be viable.
  //Until then, enjoy the magic numbers. 4 = paths count, 128 = max path length
  render_context.image_atlas.count = 4;
  ...
  for(u32 i = 0; i < render_context.image_atlas.count; i++) {
    SDL_Surface* surface = IMG_Load(texture_paths[i]);

    render_context.image_atlas.images[i].w = surface->w;
    render_context.image_atlas.images[i].h = surface->h;
    render_context.image_atlas.images[i].texture = SDL_CreateTextureFromSurface(render_context.renderer, surface);
    assert(render_context.image_atlas.images[i].texture);
...
int main(int argc, char *args[]) {
  ...
  for(u32 i = 0; i < entity_names_count; i++) {
    Entity__create(entity_names[i]);
  }

  memcpy_s(&game_states.previous_state, sizeof(GameState), &game_states.current_state, sizeof(GameState));
  memcpy_s(&game_states.render_state, sizeof(GameState), &game_states.current_state, sizeof(GameState));
  ...
  while (game_is_still_running)
  {
      ...
      {
          memcpy_s(&game_states.previous_state, sizeof(GameState), &game_states.current_state, sizeof(GameState));
          //game_states.previous_state = game_states.current_state;
          update(delta_time);
          accumulator -= delta_time;
      }

      //game_states.alpha = accumulator / delta_time;
      game_states.alpha = fmin(accumulator / delta_time, 1.0);
      render();
